 ...
 */
import type Repository from "@oinpanel/meteor-admin-sdk/es/data/repository";
import Criteria from "@oinpanel/meteor-admin-sdk/es/data/Criteria";
import { useRepository } from "@oinpanel/meteor-admin-sdk/es/data/composables/useRepository";
import { getRepository } from "@oinpanel/meteor-admin-sdk/es/data/composables/getRepository";
import MtDataTable, {
  type ColumnChanges,
...
const handleItemDelete = async ({ id }: { id: string }) => {
  ...
  if (isInIframe && !props.forceRealModal) {
    const { ui } = await import("@oinpanel/meteor-admin-sdk");
